...
/ Nov 03,'09 R0.07e Separated out configuration options from fatfs.h to ffconf.h.
...
#include "card.h"		/* Declarations of low level disk I/O functions */
...
static int enq_lock (void)	/* Check if an entry is available for a new file */
{
	UINT i;
...
static UINT inc_lock (	/* Increment file open counter and returns its index (0:int error) */
...
static FRESULT dec_lock (	/* Decrement file open counter */
...
)
{
	...
	/* vvvvv: Инициализируем SD карту ЗДЕСЬ!!! */
	if(disk_init(0)) { /* На ошибку   */
	   return FR_DISK_ERR;
	}	
	...
	FatFs[vol] = fs;			/* Register new fs object */
        ...
        /* проверить файловую систему что это FAT32!!! */
     	return  chk_mounted(&n, &rfs, 1);
}
